...
class Rules:
    ...
    #: ``ticks: 500`` read as 500 updates over ticks 0..499.
...
class _Species:
    ...
    __slots__ = ("slot", "index", "plant", "offsets", "soil_ok", "ttm",
                 ...
                 "adjacent_shade_penalty", "moore_range")
    ...
    def __init__(self, slot: int, plant: Plant, world: World, rules: Rules):
        ...
        self.moore_range = (plant.spread_range
                            if plant.spread_type == "Moore" else 0)
...
def simulate(world: World,
             ...
             species: Iterable[int] | None = None) -> FinalState:
            ...
            # ---------------- spread ---------------------------------
                ...
                for sp in specs:
                    ...
                    if rules.spread_clock == "since_planting":
                        phase_arr = a
                        due = mine & (phase_arr % sp.rate == 0)
                    else:
                        phase_arr = a - sp.ttm
                        due = mine & (phase_arr % sp.rate == 0)
                        if rules.spread_clock == "delayed_maturity":
                            due &= phase_arr >= sp.rate
                    ...
                    if (rules.shade and any_shade and rules.adjacent_shade_penalty
                        ...
                        slow = mine & near & (phase_arr % (2 * sp.rate) != 0)
                        due &= ~slow
                    if not due.any():
                        continue
                    if sp.moore_range:
                        tgt = dilate_square(due, sp.moore_range,
                                            include_centre=False)
                    else:
                        tgt = dilate_offsets(due, sp.offsets)
                ...
                if occ_needs_rank:
                    challenger_rank = rank_of_slot[best_o]
                    defender_rank = rank_of_slot[np.where(occ, sp_arr, S)]
                    take |= occ & has_o & (challenger_rank > defender_rank)
                else:
                    take |= occ & has_o & (sp_arr != best_o.astype(np.int8))
